#!/usr/bin/env bash
# validate-draft.sh — AfterTool hook for proposal and implementation validation
# Fired after write_file calls. Validates:
#   - FINAL proposal files in .gemini/proposal/
#   - Intermediate files in .gemini/proposal/drafts/
#   - Implementation files in .gemini/implementation/
#
# Gemini CLI hook protocol:
#   stdin: JSON with hook_event, tool_name, tool_input, tool_response
#   stdout: JSON with action ("continue" or "block") and message
#
# Exit codes (also respected):
#   0 = valid (or no files yet)
#   2 = invalid file detected (blocks tool completion)

set -euo pipefail

# --- Gemini CLI Hook Protocol: Read stdin ---
INPUT=$(cat)

# Extract file path from hook input (if available)
HOOK_FILE_PATH=""
if command -v python3 &>/dev/null; then
  HOOK_FILE_PATH=$(echo "$INPUT" | python3 -c "
import sys, json
try:
...
# Early exit: if we know the file path and it's not in a validated directory, skip
if [ -n "$HOOK_FILE_PATH" ]; then
  case "$HOOK_FILE_PATH" in
    .gemini/proposal/*|.gemini/implementation/*) ;; # continue validation
    *)
...
PROPOSAL_DIR="${GEMINI_PROJECT_DIR:-.}/.gemini/proposal"
...
# --- Validate FINAL proposal files in .gemini/proposal/ ---
...
IMPL_DIR="${GEMINI_PROJECT_DIR:-.}/.gemini/implementation"
...
# --- Report (Gemini CLI hook protocol: JSON output) ---

if [ -n "$ERRORS" ]; then
  # Escape errors for JSON output
  ESCAPED_ERRORS=$(echo -e "$ERRORS" | python3 -c "
import sys, json
text = sys.stdin.read().strip()
print(json.dumps(text))
" 2>/dev/null || echo "\"Validation failed\"")
  echo "{\"action\": \"block\", \"message\": ${ESCAPED_ERRORS}}"
  exit 2
fi

echo '{"action": "continue", "message": "Validation passed"}'
exit 0
